...
var something = 'test parameter'; //this is a possible way to push a user parameter to the sensor during setup
...
var device_id_callback = {
  success: function(resp){
    var id = resp['id'];
    deviceID.value = id;
    // we only need to show the deviceid stuff if it is actually needed.
    //document.getElementById('deviceid-div').style.display = 'block';
  },
  error: function(error, resp){
    console.log(error);
...
var scan = function(){
  ...
  document.getElementById('deviceid-div').style.display = 'none';
...
var configure = function(evt){
  ...
  if(network.sec != 0){
	...
	console.log('no need to encryot password');
  }
  // send
  ...
  //postRequest('http://httpbin.org/post', jsonData, configure_callback);
};

var configure_callback = {
  success: function(resp){
    console.log('Credentials received.');
    //Now connect to the WiFi (after a second)
    connectButton.innerHTML = 'Credentials received...';
    setTimeout(
		function(){postRequest(base_url+'connect-ap', {idx:0}, connect_callback);
	},1000);
